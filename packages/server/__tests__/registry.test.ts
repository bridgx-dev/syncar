...
describe('ClientRegistry', () => {
    ...
    describe('channel management', () => {
        it('should register a channel by name via subscribe', () => {
            const connection = createMockConnection('client-1')
            registry.register(connection)
        ...
        it('should get channel subscribers as Set', () => {
            ...
            const subscribers = registry.getChannelSubscribers(
                'chat' as ChannelName,
            )
    ...
    describe('subscription management', () => {
        ...
        it('should create channel if not exists when subscribing', () => {
            ...
            expect(result).toBe(true)
            expect(registry.getChannels()).toContain(
                'new-channel' as ChannelName,
            )
        })
        ...
        it('should get subscribers for a channel', () => {
            ...
            expect(subscribers.map((c) => c.id)).toEqual([
                'client-1',
                'client-2',
            ])
        })
        ...
        it('should return empty array for non-existent channel', () => {
            const subscribers = registry.getSubscribers(
                'nonexistent' as ChannelName,
            )
        ...
        it('should return 0 for non-existent channel subscriber count', () => {
            expect(
                registry.getSubscriberCount('nonexistent' as ChannelName),
            ).toBe(0)
        })
    ...
    describe('cleanup on client unregister', () => {
        it('should unsubscribe client from all channels when unregistered', () => {
            ...
            expect(
                registry.getClientChannels('client-1' as ClientId).size,
            ).toBe(0)
        })

 ...
 *
 * @remarks
 * - 'bidirectional': Clients can send and receive messages
 * - 'send-only': Clients can only send messages
 * - 'receive-only': Clients can only receive messages
 *
 * @example
 * ```ts
 * const flow: ChannelFlow = 'bidirectional'  // Send and receive
 * const flow2: ChannelFlow = 'send-only'  // Send only
 * const flow3: ChannelFlow = 'receive-only'  // Receive only
 * ```
 */
 ...
 * Controls message distribution (scope) and direction (flow).
 *
 * @property scope - Who receives: 'broadcast' (all clients) or 'subscribers' (only subscribed)
 ...
 * const broadcastChannel: ChannelOptions = {
 *   scope: 'broadcast',
 *   flow: 'bidirectional'
 * }
 *
 * // Subscriber-only channel - only subscribed clients receive messages
 * const subscriberChannel: ChannelOptions = {
 *   scope: 'subscribers',
 *   flow: 'bidirectional'
 * }
 *

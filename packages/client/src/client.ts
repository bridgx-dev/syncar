...
export class SynnelClient {
  ...
  async subscribe<T = unknown>(
    ...
    if (!subscription) {
      ...
      subscription.sendSubscribe = async (ch: ChannelName, data?: unknown) => {
      ...
      this.subscriptions.set(channel, subscription as ChannelSubscriptionImpl)
    }
  ...
  getSubscription<T = unknown>(
    ...
    return this.subscriptions.get(channel) as ChannelSubscription<T> | undefined
  }
  ...
  getSubscribedChannels(): ChannelName[] {
    return Array.from(this.subscriptions.keys()).filter(
      (channel) => this.subscriptions.get(channel)?.state === 'subscribed',
    )
  }

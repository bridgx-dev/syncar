...
describe('MessageBus', () => {
    ...
    describe('unsubscribe', () => {
        ...
        it('should unsubscribe from a channel', () => {
            ...
            expect(result).toBe(true)
            expect(bus.getChannel('test')?.hasSubscriber('client-1')).toBe(
                false,
            )
    ...
    describe('unsubscribeAll', () => {
        it('should unsubscribe from all channels', () => {
            ...
            expect(bus.getChannel('channel1')?.hasSubscriber('client-1')).toBe(
                false,
            )
            expect(bus.getChannel('channel2')?.hasSubscriber('client-1')).toBe(
                false,
            )
        })
    })

...
import type { ChannelSubscription } from '@synnel/client-v2'
import type {
  UseChannelOptions,
  UseChannelReturn,
} from './types.js'
...
export function useChannel<T = unknown>(
  ...
  const [subscriptionState, setSubscriptionState] = useState<'unsubscribed' | 'subscribing' | 'subscribed' | 'unsubscribing'>('unsubscribed')

  // Refs to store mounted state and callbacks
  const subscriptionRef = useRef<ChannelSubscription<T>>()
    ...
    const setupSubscription = async () => {
      ...
      try {
        ...
        if (!subscription || subscription.channel !== channel) {
          ...
          if (subscriptionRef.current && subscriptionRef.current.channel !== channel) {
            await subscriptionRef.current.unsubscribe()
            subscriptionRef.current = undefined
          }

          // Create new subscription with callbacks
          subscription = await client.subscribe<T>(channel, {
          ...
          setSubscriptionState(subscription.state)
        }
      } catch (error) {
        if (!mountedRef.current) return
  ...
  return {
    ...
    isConnecting: client.status === 'connecting' || client.status === 'reconnecting',
    data,
    error,
    subscriptionState,
    send,
